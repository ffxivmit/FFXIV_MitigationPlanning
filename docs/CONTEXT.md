...
- **施放紀錄重新索引（cast record reindex）** — `mitMap`/`skillStateMap`/`notesMap` 這三個以「技能實例 + 位置」為 key 的紀錄，在**隊伍成員**增刪/拖曳重排、或**自訂列**被刪除時，key 內嵌的位置片段（member 軸的 `-p{index}-`、row 軸的列索引尾段）都要跟著整批重寫，否則紀錄會錯位到別的成員/別的列。這段重映射邏輯收在 `src/castRecordReindex.js`（純函式），成員增刪/重排與自訂列刪除各自呼叫，不再各自重寫一份。除了 main.js 本機操作用的 `reindexCastRecordsByMember`/`reindexCastRecordsByRemovedRow`，另有兩個給三向合併換算座標系用的：`buildPartyRealignMapping`（兩份隊伍職業陣列之間的成員對照表）與 `remapCastRecordRows`（任意列重排的通用版本，`reindexCastRecordsByRemovedRow` 是它「刪掉單一列」的特例）。
- **座標系（coordinate system）** — 施放紀錄的位置片段是**相對的**：`p{n}` 的 `n` 只在「某一份特定的 `party` 順序」下才有意義，列索引也只在「某一份特定的 `customRowsByDuty` 排列」下才有意義。一份快照內部一定自洽，但**兩份快照的同一個 key 未必指同一個人／同一列**。任何要跨快照比對施放紀錄的地方（目前只有三向合併），都必須先把各份快照換算到同一套座標系才能逐 key 比對，否則等於把不同單位的數字相加。同職業有多位時（例如三名騎士）換算會存在多組合法解，但這些解彼此等價——模糊只發生在職業相同的成員之間，而同職業技能表一致，任一組解渲染結果都正確，因此固定採「同職業依出現順序配對」這組確定性的解。
- **三向合併（three-way merge）** — 共同編輯（Supabase edit token）儲存時遇到別人也存過的情況，把 `base`（上次讀取的計畫快照）／`dbData`（伺服器目前版本）／`local`（本機未儲存版本）三份快照比對：不衝突的欄位自動合併，同一欄位雙方都改且結果不同才標記為衝突、交給使用者在對話框裡選。這段純函式邏輯收在 `src/planMerge.js`：`mergePayloads(base, dbData, local, { dutyRowCounts })` 算出自動合併結果與衝突清單，`applyConflictChoices(autoMerged, enriched, localData, { dutyRowCounts })` 把使用者在對話框對每個衝突欄位選的 `local`/`db` 套用上去、回傳最終版本（不 mutate 輸入）。**執行順序不能改**：一定要先算出合併後的 `party` 與 `customRowsByDuty`，把三份快照全部換算到那套座標系，才做 `mits`/`notes` 的逐 key 比對（見上面「座標系」）；`dutyRowCounts`（各副本原始時間軸列數）是換算列索引時用來分辨「哪些索引屬於自訂列」的，這個數字不在計畫快照裡，只能由 main.js 提供。`mergePayloads` 回傳的 `rebased` 是換算後的三份快照，conflicts 的 key 屬於這套座標系，呼叫端要拿 `rebased.local`/`rebased.db` 去查值。**不處理**衝突的顯示格式化——把 `{ type, key }` 轉成中文標籤（技能名稱、職業名稱等）給對話框顯示的邏輯（`_enrichConflicts`）留在 main.js，跟減傷帳本「不處理顯示格式」是同一個原則。
